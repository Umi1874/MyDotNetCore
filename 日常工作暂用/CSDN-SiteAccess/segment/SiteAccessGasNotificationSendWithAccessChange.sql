...
FROM
        ...
        AND crmas.FrmpDate IS NOT NULL --AND crmas.CreatedDate > '2020-01-01'
        ...
        AND crmass.[Name] NOT IN (
                'Account Disconnected Non Payment',
                'Closed',
                'Account Consolidation',
                'Account Closed to work around system'
        )
        INNER JOIN crm.Site crmsite ON crmsite.SiteId = crmas.SiteId
...
WHERE
        NOT EXISTS (
                SELECT
                        1
                FROM
                        aemo.AseXmlTransaction aemoaxt
                WHERE
                        aemoaxt.AccountServiceId = crmas.AccountServiceId
